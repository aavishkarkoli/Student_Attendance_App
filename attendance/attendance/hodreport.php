		...
		<div class="col-md-12 col-lg-12">
			<form action="" method="GET" class="form-inline" data-toggle="validator">
				<div class="form-group">
					...
					INNER JOIN faculty_subject WHERE faculty_subject.id = subject.id AND faculty_subject.uid = $suid  ORDER BY subject.name";
							...
							$normalized_weekday = strtolower($weekday);
							 if(($normalized_weekday!="saturday") && ($normalized_weekday!="sunday"))
							 {
								...
								//print_r($result);
									...
									{
										$present++;
										echo "<td><a class='text-success' href='attendance.php?subject=" . $selsub . "&date=" . $currentDate . "'>Present</a></td>";
									}
									else
									{
										echo "<td><a class='text-danger' href='attendance.php?subject=" . $selsub . "&date=" . $currentDate . "'>Absent</a></td>";
										$absent++;
									}
								}else
								{
									echo "<td><a href='attendance.php?subject=" . $selsub . "&date=" . $currentDate . "'>TakeAttendance</a></td>";
								}
							}
						}
						if($totlec!=0)
